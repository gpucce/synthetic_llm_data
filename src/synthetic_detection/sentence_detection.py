...
def main(args):
    ...
    if args.real_data_path is not None:
        ...
        # args..prompt = PROMPT_REGISTRY[self.project][self.preprocessing][self.model_name]
        ...
        real_dataset = real_dataset.map(preprocessing_fn, desc="Generating prompts").shuffle(seed=args.seed)
        ...
        assert len(_human_dataset) == len(_synth_dataset), f"{len(_human_dataset)} != {len(_synth_dataset)}"
        ...
        _dataset = _dataset.map(lambda x: {args.col_name: " ".join(x[args.col_name].split(" ")[:150])})
    ...
    ds = ds.map(
        lambda x: tokenizer(x[args.col_name], padding="max_length", truncation=True, max_length=512), batched=True)
