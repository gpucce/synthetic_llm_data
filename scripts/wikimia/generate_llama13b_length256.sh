...
srun python -m synthetic_llm_data.src.synthetic_detection.detect_gpt.dataset_lm_modification \
    ...
    --data-path /leonardo_scratch/large/userexternal/gpuccett/datasets/wikimia/WikiMIA_length256.json \
    --output-path /leonardo_scratch/large/userexternal/gpuccett/data/wikimia_experiments/llama-2-13b-hf_length256 \
    --seed 1 \
    --col-name input \
    --max-new-tokens 250 \
    --min-new-tokens 200 \
